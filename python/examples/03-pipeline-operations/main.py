#!/usr/bin/env python3
"""
Pipeline Operations Example - Moderately AI Python SDK

This example demonstrates the complete pipeline operations workflow using the SDK,
showing the exact method mappings from REST API endpoints to SDK methods.

REST API vs SDK Method Mapping:
- REST: POST /pipelines
  SDK:  client.pipelines.create(name="Pipeline Name")

- REST: POST /pipeline-configuration-versions
  SDK:  pipeline.create_configuration_version(configuration={...})

- REST: POST /pipeline-executions
  SDK:  config_version.execute(pipeline_input={...}) or pipeline.execute({...})

- REST: GET /pipeline-executions/{id}
  SDK:  client.pipeline_executions.retrieve(execution_id)

- REST: GET /pipelines
  SDK:  client.pipelines.list()

- REST: DELETE /pipelines/{id}
  SDK:  pipeline.delete() or client.pipelines.delete(pipeline_id)
...
def create_sample_pipeline_config():
    ...
    pipeline_config = {
        "id": "echo_demo",
        "name": "Echo Demo",
        "description": "Simple echo pipeline that demonstrates external input injection. Takes a string input from CLI and outputs it unchanged. Pipeline flow: External Input → Input Block → Output Block. Type flow: string → string",
        "version": "1.0.0",
        "blocks": {
            "user_input": {
                "id": "user_input",
...
def main():
    """Demonstrate complete pipeline operations workflow."""
    print("🚀 Pipeline Operations Example - Moderately AI SDK")
    print("=" * 60)
    ...
    try:
        ...
        #
        # Returns: PipelineModel instance with rich methods
        ...
        # SDK Method: pipeline.create_configuration_version(configuration, **kwargs)
        #
        ...
        # SDK Method: config_version.validate()
        #
        # This maps to: POST /pipeline-configuration-versions/validate
        # { "configuration": {...} }
        #
        # Returns: Dict with validation results
        validation_result = config_version.validate()
        ...
        #
        # Returns: PipelineExecutionModel instance
        ...
        # SDK Method: execution.get_output()
        #
        ...
        # SDK Method: client.pipeline_executions.list(**filters)
        #
        ...
        #
        # Returns: Dict with "items" (list of PipelineExecutionModel) and "pagination" metadata
        ...
        # SDK Method: pipeline.list_configuration_versions()
        #
        # This maps to: GET /pipeline-configuration-versions?pipelineIds=[{pipeline_id}]
        #
        # Returns: List of PipelineConfigurationVersionModel instances
        config_versions = created_pipeline.list_configuration_versions()
        ...
        #
        # Returns: New PipelineConfigurationVersionModel instance (cloned)
        cloned_version = config_version.clone()
        ...
        # SDK Method: client.pipelines.list(**filters)
        #
        ...
        #   order_direction: "asc" or "desc"
        #
        # Returns: Dict with "items" (list of PipelineModel) and "pagination" metadata
        ...
        print("\n1️⃣1️⃣ Pipeline Information & Rich Methods...")

        # PipelineModel provides rich properties and methods without additional API calls:
        # Properties: pipeline_id, name, description, team_id, created_at, updated_at
        # Rich methods: create_configuration_version(), list_configuration_versions(), execute()
        ...
        # SDK Method: pipeline.update(name=None, description=None, **kwargs)
        #
        ...
        created_pipeline = client.pipelines.retrieve(created_pipeline.pipeline_id)
        ...
        print("\n1️⃣3️⃣ Alternative Execution Methods...")
        print("   ✅ Blocking execution (demonstrated above)")
        print("   Alternative: Non-blocking execution would use:")
        print("   execution = config_version.execute(input_data, summary, block=False)")
        print("   then use execution.refresh() to monitor progress manually")

        print("\n🎉 Pipeline operations completed successfully!")
        ...
        print("  • pipeline.list_configuration_versions() - List versions")
        print("  • client.pipelines.list() - List pipelines with filtering")
        print("  • pipeline.update() - Update pipeline properties")
        ...
        if created_pipeline:
            try:
                ...
                # Alternative: client.pipelines.delete(pipeline_id)
                created_pipeline.delete()
